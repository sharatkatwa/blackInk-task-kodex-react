  ...
  return (
    <div className="absolute top-[60%] right-0   mt-2 w-40 bg-[var(--bg)] rounded-lg shadow-lg border border-gray-500/40 z-50 transition-all duration-300 text-[var(--text)]">
      <div className="p-2">
        ...
        <button onClick={handleUpdate} className="w-full text-left px-3 py-2 hover:bg-primary/50 rounded flex items-center gap-5 cursor-pointer border-b border-gray-500/20">
          {!article.published ? (
            <>
              {" "}
        ...
        <button onClick={() => deleteArticle(article.id)} className="w-full text-left px-3 py-2 hover:bg-primary/50 text-red-500 rounded flex items-center gap-5 cursor-pointer">
